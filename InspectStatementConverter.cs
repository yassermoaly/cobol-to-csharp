...
namespace CobolToCSharp
{
    public class InspectStatementConverter : IStatementConverter
    {
        ...
        {
            if(new Regex($"{"INSPECT".RegexUpperLower()}[ ]+[a-zA-Z][a-zA-Z0-9-]+[ ]+{"REPLACING".RegexUpperLower()}[ ]+{"ALL".RegexUpperLower()}[ ]+[Xx]*\"[0-9A-Za-z ]+\"[ ]+{"BY".RegexUpperLower()}[ ]+[Xx]*\"[0-9A-Za-z ]+\"").IsMatch(Line))
            {
                ...
                Match ReplaceMatch = new Regex($"{"ALL".RegexUpperLower()}[ ]+[Xx]*\"[0-9A-Za-z ]+\"[ ]+{"BY".RegexUpperLower()}[ ]+[Xx]*\"[0-9A-Za-z ]+\"").Match(Line);
                ...
                Regex ReplaceValue = new Regex("\"[0-9A-Za-z ]+\"");
                ...
                return $"{Variable} = {Variable}.Replace({ReplaceFromValue},{ReplaceFromIsHexa.ToString().ToLower()},{ReplaceToValue},{ReplaceToIsHexa.ToString().ToLower()});";
            }
